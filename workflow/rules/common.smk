...
def get_fast5(wildcards):
    res = samples_df[
        (samples_df["condition"] == wildcards.cond)
        & (samples_df["replicate"] == int(wildcards.rep))
    ]
...
def get_threads(config, rule_name, default=1):
    try:
        ...
        logger.error(
            "Could not find value `threads` for rule `{}` in config file".format(
                rule_name
            )
        )
        return default
...
def get_opt(config, rule_name, default=""):
    try:
        ...
        logger.error(
            "Could not find value `opt` for rule `{}` in config file".format(rule_name)
        )
        return default
...
def get_mem(config, rule_name, default=1000):
    try:
        ...
        logger.error(
            "Could not find value `mem_mb` for rule `{}` in config file".format(
                rule_name
            )
        )
        return default

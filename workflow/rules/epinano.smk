...
rule merge_fastq_epinano:
    input:
        fastq_dir=rules.ont_guppy_epinano.output.fastq_dir,
    output:
...
rule alignmemt_postfilter_epinano:
    ...
    container:
        "library://aleg/default/pybiotools:0.2.7"
    shell:
        "pyBioTools Alignment Filter {params.opt} -i {input.bam} --select_ref_fn {input.ref_list} -o {output.bam} -l {output.selected_reads_fn} --verbose &> {log}"
...
rule alignmemt_merge_epinano:
    ...
    container:
        "library://aleg/default/minimap2:2.17"
    shell:
        "samtools merge {params.opt} -@ {threads} {output.bam} {input.bam_list} &> {log} && samtools index {output.bam} &> {log}"
...
rule epinano_splitbam:
    ...
    container:
        "library://aleg/default/pybiotools:0.2.7"
    shell:
        "pyBioTools Alignment Split --index -i {input.bam} -n 10 --output_fn_list {output}"
...
rule epinano_filter_rrach_variants:
    ...
    container:
        "library://aleg/default/epinano:1.2.0"
    script:
        "../scripts/epinano_filter_kmers.py"
...
rule epinano_gather_variants:
    ...
    output:
        filteredvariants=join(
            ...
            module_name,
            rule_name,
            "{cond}.all.plus_strand.per_site.5mer.csv",
        ),
    log:
        join("logs", module_name, rule_name, "{cond}.log"),
    threads: get_threads(config, rule_name)
...
rule epinano_delta_variants:
    input:
        control_variants=join(
            "results",
            module_name,
            "epinano_gather_variants",
            "control.all.plus_strand.per_site.5mer.csv",
        ),
        test_variants=join(
            "results",
            module_name,
            "epinano_gather_variants",
            "test.all.plus_strand.per_site.5mer.csv",
        ),
    output:
...
rule epinano_postprocess:
    input:
        join(
            ...
            module_name,
            "epinano_predict",
            "test.q3.mis3.del3.MODEL.rrach.q3.mis3.del3.linear.dump.csv",
        ),
        join(
            "results",
            module_name,
            "epinano_predict",
            "control.q3.mis3.del3.MODEL.rrach.q3.mis3.del3.linear.dump.csv",
        ),
        join(
            "results",
            module_name,

...
rule tombo_level_sample_compare:
    input:
        control_fast5=expand(
            join("results", module_name, "tombo_preprocess", "control_{rep}"),
            rep=replicates_list,
        ),
        test_fast5=expand(
            join("results", module_name, "tombo_preprocess", "test_{rep}"),
            rep=replicates_list,
        ),
    output:
    ...
    container:
        "library://aleg/default/tombo:1.5.1"
    shell:
        "tombo detect_modifications level_sample_compare {params.opt} --processes {threads} --fast5-basedirs {input.test_fast5} --alternate-fast5-basedirs {input.control_fast5} --statistics-file-basename {params.bn} &> {log}"

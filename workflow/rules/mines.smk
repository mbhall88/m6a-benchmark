...
rule tombo_de_novo:
    input:
        fast5=expand(
            join("results", "tombo", "tombo_preprocess", "test_{rep}"),
            rep=replicates_list,
        ),
    output:
    ...
    shell:
        "tombo detect_modifications de_novo {params.opt} --processes {threads} --fast5-basedirs {input.fast5} --statistics-file-basename {params.bn} &> {log}"
...
rule tombo_de_novo_text_output:
    ...
    output:
        ...
        fraction=temp(
            join(
                ...
                module_name,
                rule_name,
                "test.fraction_modified_reads.plus.wig",
            )
        ),
        coverage_minus=temp(
            join("results", module_name, rule_name, "test.coverage.minus.bedgraph")
        ),
        fraction_minus=temp(
            join(
                "results",
                module_name,
                rule_name,
                "test.fraction_modified_reads.minus.wig",
            )
        ),
    log:
        join("logs", module_name, rule_name, "test.log"),
    ...
    container:
        "library://aleg/default/tombo:1.5.1"
    shell:
        "tombo text_output browser_files --fast5-basedirs {input.fast5} --statistics-filename {input.stats_h5} --browser-file-basename {params.bn} --file-types coverage fraction &> {log}"
...
rule mines_wig2bed:
    ...
    output:
        fraction=join(
            "results", module_name, rule_name, "test.fraction_modified_reads.plus.bed"
        ),
    log:
    ...
    shell:
        "wig2bed < {input.fraction} > {output.fraction} 2> {log}"

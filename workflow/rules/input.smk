...
rule get_transcriptome:
    ...
    script:
        f"../scripts/get_transcriptome.py"
...
rule generate_transcriptome_picard_index:
    input:
        fasta=rules.get_transcriptome.input.fasta,
    output:
        picard_dict=join(
            "results",
            module_name,
            "get_transcriptome",
            "transcriptome_reference.fa.dict",
        ),
    log:
        join("logs", module_name, rule_name, "transcriptome_reference_picard.log"),
    threads: 1
    resources:
        mem_mb=get_mem(config, rule_name),
    container:
    ...
    shell:
        """
        tmpfa=$(mktemp --suffix ".fasta")
        perl -pe "s/'$/primesymbol/" {input.fasta} > "$tmpfa" 2> {log}
        java -jar /usr/picard/picard.jar CreateSequenceDictionary \
          -R "$tmpfa" -O {output.picard_dict}.sanitized 2>> {log}
        perl -pe "s/primesymbol/'/" {output.picard_dict}.sanitized > {output.picard_dict} 2>> {log}
        """
